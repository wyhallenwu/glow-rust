...
- 分栏式终端 UI，包含可折叠目录树、即时过滤、Git 分支切换、文档预览、文件变化监听、窄终端布局和可选鼠标操作。
...
打开目录会进入 TUI。宽度达到 88 列时显示目录树和预览双栏；更窄时一次显示一个面板，通过 `Tab` 或 `Enter` 切换。如果该目录位于 Git 工作区内且系统可以执行 `git`，顶部会显示当前分支；按 `b` 可从本地分支列表中选择并切换。切换后文档树和预览会立即重新扫描；若未提交改动阻止切换，Git 的错误会显示在状态栏中。
...
| `b` | 打开本地 Git 分支选择器 | 打开本地 Git 分支选择器 |
| `Esc` | 清除过滤；没有过滤时退出 | 同左 |
| `r` | 立即重新扫描 | 立即重新扫描 |
| `?` | 打开/关闭帮助 | 打开/关闭帮助 |
| `q` / `Ctrl-C` | 退出 | 退出 |
...
分支选择器中可使用 `↑` / `↓` 或 `j` / `k` 移动，`Enter` 执行 `git switch`，`Esc`、`b` 或 `q` 取消。这里只列出本地分支，不会自动 fetch、创建分支或丢弃工作区改动。
